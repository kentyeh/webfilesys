...
public class UserListRequestHandler extends AdminRequestHandler {
	...
	public UserListRequestHandler(HttpServletRequest req,
	...
	protected void process() {
		...
		if (initial == null) {
			...
			if (pageSizeParm != null) {
				...
				Integer userListPageSize = (Integer) session
		...
		if (initial == null) {
			...
			if (filter != null) {
				...
				String userListFilter = (String) session
						.getAttribute(SESSION_KEY_USER_LIST_FILTER);
		...
		output.println("<input type=\"text\" name=\"searchMask\" size=\"10\" maxlength=\"32\" value=\""
				+ searchMask + "\" style=\"width:80px\">");
		output.println("</td>");
		...
		output.println("<input type=\"text\" name=\"" + Paging.PARAM_PAGE_SIZE
				+ "\" maxlength=\"3\" maxlength=\"3\" value=\"" + pageSize
				+ "\" style=\"width:40px\">");
		...
		if (!searchMask.trim().isEmpty()) {
			for (int i = allUsers.size() - 1; i >= 0; i--) {
				...
				if ((!CommonUtils.containsString(user.getUserid(), searchMask))
						&& (!CommonUtils.containsString(user.getLastName(),
								searchMask))
						&& (!CommonUtils.containsString(user.getFirstName(),
								searchMask))
						&& (!CommonUtils.containsString(user.getEmail(),
								searchMask))) {
					allUsers.remove(i);
				}
			}
		...
		ArrayList<TransientUser> usersOnPage = paging.getElementList();
            ...
            for (TransientUser actUser : usersOnPage) {
                ...
                output.print("<a href=\""+ req.getContextPath() + "/servlet?command=admin&cmd=editUser&username="
		...
		if (paging.getElementNumber() > 0) {
			...
			if (paging.isFirstPage()) {
				...
				output.print("<a href=\""+ req.getContextPath() + "/servlet?command=admin&cmd=userList&startIndex="
			...
			output.println(paging.getStartIndex() + " - "
					+ paging.getEndIndex() + " of " + paging.getElementNumber());
			...
			for (int k = 0; k < startIndices.size(); k++) {
				...
				if (idx != paging.getStartIndex() - 1) {
					if (((pageCounter - 1) % pageStep == 0)
						...
						output.print("<a class=\"fn\" href=\""+ req.getContextPath() + "/servlet?command=admin&cmd=userList&startIndex="
								+ idx + "\">");
