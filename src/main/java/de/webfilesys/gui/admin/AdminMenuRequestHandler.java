...
public class AdminMenuRequestHandler extends AdminRequestHandler
{
	...
	protected void process()
	{
		...
		if (WebFileSys.getInstance().isMaintananceMode())
		{
			...
			output.println("<a href=\"" + req.getContextPath() + "/servlet?command=admin&cmd=switchMode\">Back to normal Operation</a>");
		}
		else
		{
			output.println("<span class=\"icon-font icon-tool iconAdminMenu\"></span>");
			output.println("<a href=\"" + req.getContextPath() + "/servlet?command=admin&cmd=switchMode\">Switch to Maintanance Mode</a>");
		}
		output.println("</div>");
