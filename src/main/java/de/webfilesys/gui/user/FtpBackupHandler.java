...
{
    private static final Logger logger = LogManager.getLogger(FtpBackupHandler.class);
	...
	protected void process()
	{
		...
		{
			alertText.append(getResource("alert.missingFtpServer","FTP Server is a required field")).append("\\n");
        }
		...
		{
			alertText.append(getResource("alert.missingFtpUserid","Userid is a required field")).append("\\n");
        }
		...
		{
			alertText.append(getResource("alert.missingFtpPassword","Password is a required field")).append("\\n");
        }
        ...
        try
        {
            ...
            logger.debug("FTP connect to " + ftpServerName + " response: " + ftpClient.getReplyString());
			...
			if (FTPReply.isPositiveCompletion(reply)) 
			{
				if (ftpClient.login(userid, password))
				{
                    ...
                    else
                    {
						...
						if (!ftpClient.changeWorkingDirectory(subDir))
						{
							...
							{
                                logger.warn("FTP cannot create remote directory " + subDir);
                                remoteChdirOk = false;
							}
							else
							{
                                logger.debug("FTP created new remote directory " + subDir);

								if (!ftpClient.changeWorkingDirectory(subDir))
								{
                                    logger.warn("FTP cannot chdir to remote directory " + subDir);
                                    remoteChdirOk = false;
                                }
                            }
                        }
				...
				{
                    logger.info("FTP connect to " + ftpServerName + " login failed");
                    javascriptAlert("Login to FTP server " + ftpServerName + " failed");
                }
			}
			else
			{
                logger.warn("FTP connect to " + ftpServerName + " response: " + reply);
                javascriptAlert("Login to FTP server " + ftpServerName + " failed");
            }
    ...
    throws IOException
    {
                ...
                for (File localFile : localFileList) {
                    if (localFile.isDirectory())
                    {
                        if (recursive)
                        {
                            ...
                            if (remoteChdirOk)
                            {
                                ...
                                {
                                    logger.warn("FTP cannot chdir .. from " + subDir);
                                    return(false);
                                }
                            }
                            else
                    ...
                    else
                    {
                        ...
                        try(FileInputStream fin = new FileInputStream(localFile))
                        {
                            ...
                            {
                                logger.warn("FTP put file " + localPath + "/" + localFile.getName() + " failed");

                                error = true;
                            }
                            else
                            {
                                logger.debug("FTP put of file " + localPath + "/" + localFile.getName() + " successful");
                        ...
                        {
                            logger.warn("FTP put file " + localPath + "/" + localFile.getName() + " failed: " + ioex);
		...
		{
            logger.warn("FTP local directory " + localPath + " is not readable");
