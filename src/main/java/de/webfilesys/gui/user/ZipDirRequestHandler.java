...
public class ZipDirRequestHandler extends UserRequestHandler {
	...
	protected void process() {
		...
		try {
	        ...
	        if (mobile != null) {
	            output.println("<input type=\"button\" value=\"" + getResource("button.return","Return") + "\" onclick=\"window.location.href='"+ req.getContextPath() + "/servlet?command=mobile&cmd=folderFileList&absPath=" + UTF8URLEncoder.encode(currentPath) + "'\">");
	        } else {
	            output.println("<input type=\"button\" value=\"" + getResource("button.return","Return") + "\" onclick=\"window.location.href='"+ req.getContextPath() + "/servlet?command=exp&expand=" + UTF8URLEncoder.encode(currentPath) +"&fastPath=true'\">");
	        }
			...
			if (isMobile()) {
	            output.println("location.href='"+ req.getContextPath() + "/servlet?command=mobile&cmd=folderFileList&absPath=" + UTF8URLEncoder.encode(currentPath) + "';");
			} else {
	            output.println("location.href='"+ req.getContextPath() + "/servlet?command=exp&expand=" + UTF8URLEncoder.encode(currentPath) + "';");
			}
